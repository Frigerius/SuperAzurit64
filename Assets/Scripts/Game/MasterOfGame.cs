...
public class MasterOfGame : MonoBehaviour, IScoreUploader, IAIObserver, IMasterOfGame
{
    ...
    void Update()
    {
        if (player.IsAlive())
        {
            ...
            time.text = string.Format("Time: {0:00}:{1:00}", timePlayed.Minutes, timePlayed.Seconds);
    ...
    public Transform SpawnAI(Vector3 position, Transform aiPrefab, AISettings settings, Image compassPre)
    {
        ...
        Image compass = Instantiate(compassPre);
    ...
    public void AddScore(int points)
    {
        ...
        highscore.text = string.Format("Score: {0:0000}", score);
    }
    ...
    public void EndOfGame()
    {
        if (!endOfGame)
        {
            ...
            scoreTxt.text = string.Format("{0:0000}", score);
            timeTxt.text = string.Format("{0:00}:{1:00}", timePlayed.Minutes, timePlayed.Seconds);
